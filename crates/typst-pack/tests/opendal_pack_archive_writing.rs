...
use typst_pack::opendal::pack_archive::{PackArchiveReadRequest, read_pack_archive};
use typst_pack::opendal::write::{
    OpenDalWritePhase, PackArchiveWriteErrorCause, PackArchiveWriteRequest,
    PackArchiveWriteRequestError, WriteKeyOutcome, WritePolicy, write_pack_archive,
};
use typst_pack::opendal::{
    Location, LocationRoleError, OperatorBinding, OperatorBindings, OperatorResolver,
};
use typst_pack::pack_archive::{CommitCertainty, DecodeLimits, ReadLimits, decode};
...
fn request_accepts_exact_objects_and_rejects_prefixes() {
    ...
    for (destination, expected) in [
        ...
        let error =
            PackArchiveWriteRequest::new(destination.clone(), WritePolicy::OverwriteExactKeys)
                .unwrap_err();
...
fn overwrite_writes_exact_borrowed_bytes_once_without_reading_or_touching_other_objects() {
    ...
    let receipt = expect_ready(pin!(write_pack_archive(&bindings, &request, &archive))).unwrap();

    assert_eq!(archive.as_slice().as_ptr(), archive_address);
    assert_eq!(receipt.destination(), request.destination());
    assert_eq!(receipt.policy(), WritePolicy::OverwriteExactKeys);
    assert_eq!(receipt.outcome(), WriteKeyOutcome::Written);
    assert_eq!(
...
fn create_or_verify_compares_empty_and_exact_streams_as_read_only_successes() {
    for (bytes, chunks) in [
        ...
        let receipt = expect_ready(pin!(write_pack_archive(
            &bindings,
            &request(WritePolicy::CreateOrVerify),
            &archive,
        )))
        .unwrap();
    ...
    for (observed, expected_at_least) in [(b"abc".as_slice(), 3), (b"abcde".as_slice(), 5)] {
        ...
        let service = WriteService::new(
            ...
            [WriteReadScript::new(
                "packs/document.typk",
                1,
                [WriteReadStep::chunk(0..observed.len())],
            )
            .unwrap()],
            [],
            16,
        ...
        let error = expect_ready(pin!(write_pack_archive(
            &bindings,
            &request(WritePolicy::CreateOrVerify),
            &archive,
        )))
        ...
        assert!(matches!(
            error.cause(),
            PackArchiveWriteErrorCause::ByteConflict {
    ...
    let service = WriteService::new(
        ...
        [WriteReadScript::new(
            ...
            [
                ...
                WriteReadStep::mutate(DestinationMutation::set("packs/document.typk", b"YYin")),
...
fn create_or_verify_creates_absent_bytes_and_performs_one_bounded_race_verification() {
    ...
    let created_service = WriteService::new(
        WriteCapabilities::all(),
        [],
        [WriteReadScript::new(
            "packs/document.typk",
    ...
    let created = expect_ready(pin!(write_pack_archive(
        &created_bindings,
        &request(WritePolicy::CreateOrVerify),
        &archive,
    )))
    ...
    let race_service = WriteService::new(
        ...
        [
            ...
            WriteReadScript::new(
                "packs/document.typk",
                1,
                [WriteReadStep::chunk(0..archive.as_slice().len())],
            )
            .unwrap(),
        ],
        [WriteScript::new(
    ...
    let race_request = request(WritePolicy::CreateOrVerify);
    let receipt = {
        let mut write = pin!(write_pack_archive(&race_bindings, &race_request, &archive));
...
fn capability_and_size_failures_are_typed_and_have_no_effect() {
    ...
    for (policy, capabilities) in [
        ...
        let service = WriteService::new(capabilities, [], [], [], 8);
        let bindings = bindings(&service);
        let error = expect_ready(pin!(write_pack_archive(
            &bindings,
            &request(policy),
        ...
        assert!(matches!(
            error.cause(),
            PackArchiveWriteErrorCause::UnsupportedPolicy { policy: actual }
                if *actual == policy
        ));
    ...
    let error = expect_ready(pin!(write_pack_archive(
        &bindings,
        &request(WritePolicy::OverwriteExactKeys),
        &archive,
    )))
    .unwrap_err();
    assert!(matches!(
        error.cause(),
        PackArchiveWriteErrorCause::UnsupportedObjectSize { byte_length: 5 }
    ));
    assert!(service.log().entries().is_empty());
}
...
fn resolver_read_create_race_and_direct_write_failures_retain_typed_causes() {
    ...
    let resolve = expect_ready(pin!(write_pack_archive(
        &FailingResolver,
        &create_request,
        &archive,
    )))
    ...
    let PackArchiveWriteErrorCause::ResolveOperator(source) = resolve.cause() else {
    ...
    assert!(cause.is::<PackArchiveWriteErrorCause>());
    ...
    let read = expect_ready(pin!(write_pack_archive(
        &read_bindings,
        &create_request,
        &archive,
    )))
    ...
    assert!(matches!(
        read.cause(),
        PackArchiveWriteErrorCause::PreflightRead(source)
    ...
    let create = expect_ready(pin!(write_pack_archive(
        &create_bindings,
        &create_request,
        &archive,
    )))
    ...
    assert!(matches!(
        create.cause(),
        PackArchiveWriteErrorCause::ConditionalCreate(source)
            if source.kind() == opendal::ErrorKind::PermissionDenied
    ));
    ...
    let race_bindings = bindings(&race_service);
    let race = {
        let mut write = pin!(write_pack_archive(
            &race_bindings,
            &create_request,
            &archive,
        ));
    ...
    assert!(matches!(
        race.cause(),
        PackArchiveWriteErrorCause::RaceVerification(source)
            if source.kind() == opendal::ErrorKind::PermissionDenied
    ));

    let direct_service = WriteService::new(
        WriteCapabilities::all(),
        [],
        [],
        [WriteScript::write_failure(
    ...
    let direct = expect_ready(pin!(write_pack_archive(
        &direct_bindings,
        &request(WritePolicy::OverwriteExactKeys),
        &archive,
    )))
    .unwrap_err();
    assert_eq!(direct.phase(), OpenDalWritePhase::DirectWrite);
    assert_eq!(direct.commit_certainty(), CommitCertainty::Indeterminate);
    assert!(matches!(
        direct.cause(),
        PackArchiveWriteErrorCause::DirectWrite(source)
            if source.kind() == opendal::ErrorKind::PermissionDenied
    ));
...
#[test]
fn dropping_write_preserves_caller_bytes_for_full_replay() {
    ...
    let overwrite = request(WritePolicy::OverwriteExactKeys);
    {
        let mut write = pin!(write_pack_archive(
            &configured_bindings,
            &overwrite,
    ...
    expect_ready(pin!(write_pack_archive(
        &replay_bindings,
        &overwrite,
        &archive,
    )))
...
#[test]
fn memory_write_replay_and_read_preserve_exact_bytes_before_decode() {
    ...
    let overwrite =
        PackArchiveWriteRequest::new(destination.clone(), WritePolicy::OverwriteExactKeys).unwrap();
    expect_ready(pin!(write_pack_archive(&bindings, &overwrite, &archive))).unwrap();

    let replay =
        PackArchiveWriteRequest::new(destination.clone(), WritePolicy::CreateOrVerify).unwrap();
    for _ in 0..2 {
        let receipt = expect_ready(pin!(write_pack_archive(&bindings, &replay, &archive))).unwrap();
        assert_eq!(receipt.outcome(), WriteKeyOutcome::AlreadyMatching);
    }

    let read_request =
        PackArchiveReadRequest::new(destination, ReadLimits::reference_v1()).unwrap();
    let read_archive = expect_ready(pin!(read_pack_archive(&bindings, &read_request))).unwrap();
    assert_eq!(read_archive.as_slice(), archive.as_slice());
    assert!(decode(&read_archive, DecodeLimits::reference_v1()).is_err());
    assert_eq!(read_archive.as_slice(), archive.as_slice());

    let changed_bytes = b"destination changed after write and is longer";
    expect_ready(pin!(
        bindings
    ...
    .unwrap();
    let changed = expect_ready(pin!(read_pack_archive(&bindings, &read_request))).unwrap();
...
fn outer_diagnostics_are_safe_and_operator_bindings_make_a_send_future() {
    ...
    let bindings = bindings(&service);
    assert_send(write_pack_archive(&bindings, &request, &archive));
}
...
fn request(policy: WritePolicy) -> PackArchiveWriteRequest {
    PackArchiveWriteRequest::new("archive:/packs/document.typk".parse().unwrap(), policy).unwrap()
}
