...
Assembly, Pack Archive acquisition and publication, compilation input
acquisition, Pack Extraction publication, package-cache publication, and
Compilation Output Artifact publication. These workflows hand exact bytes to or
...
the shared acquisition work in #216 and #223 uses core path validation and
construction seams, the publication execution in #197 applies existing plans
...
and publication plans. Core semantic modules never depend on OpenDAL.

Shared acquisition, publication, and scheduling mechanics remain crate-private.
...
operation that cannot honor a requested publication policy reports
...
turn core path validation, package and font layout, construction, publication
...
`package-acquisition` features already ship first-party adapter and acquisition
...
does not own a runtime. Typst-pack owns bounded in-flight acquisition fan-out
...
- `opendal::raw::Access` is implemented by #209's deterministic listing/read
  test service and #217's deterministic publication test service.
...
Enabling `opendal` changes Engine Identity and Exporter Identity because
...
`PublicationPolicy`, operation-specific `...Source` entries, and the acquisition,
progress, receipt, and outcome families are adapter vocabulary. They describe
how bytes are addressed and moved, not what a Pack is. Their contracts belong in
the OpenDAL integration specification's Adapter vocabulary rather than
