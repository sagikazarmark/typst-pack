...
**Transport Operation**:
An acquisition, encoding, projection, publication, spooling, or delivery operation behind one role-specific adapter interface. Every Transport Operation admits its capabilities, Deployment Trust Profile, budgets, interruption controls, and required commit and cleanup strengths before freezing a complete plan; it then transfers boundedly, verifies exact identities, commits once, and returns one Transport Receipt. Cancellation or deadline before commit wins, while a later signal cannot rewrite committed success.

**Transport Receipt**:
The structured operational evidence returned by a Transport Operation. A common envelope records requested and admitted controls, exact byte counts and Content Identities, stage reached, commit and cleanup facts, optional timing, and sanitized adapter class; a role-specific payload records Pack, tree, dependency, projection, publication, or delivery facts. Transport Locators, credentials, and raw adapter detail appear only in separately capability-gated sensitive projections.
...
**Transport Cleanup Strength**:
The minimum partial-state cleanup guarantee requested before a Transport Operation is admitted and the actual result reported afterward. A staged adapter performs bounded cleanup before return and reports either complete cleanup or a capability-gated residual Transport Locator; a streaming sink reports already exposed bytes as non-retractable. Cleanup never continues as hidden background work or overwrites the operation's primary failure.
